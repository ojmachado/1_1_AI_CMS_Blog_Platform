...
export const PostPage: React.FC = () => {
  ...
  const processedContent = useMemo(() => {
      if (!post) return '';
      // Remove redundância de título se a IA gerou repetido no topo do markdown
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center gap-4 bg-white">
      ...
      <p className="text-slate-400 text-xs font-black uppercase tracking-widest">Carregando Insight...</p>
    </div>
  );
  ...
  const mdComponents = {
    ...
    blockquote: ({ children }: any) => (
      <blockquote className="border-l-4 border-indigo-500 bg-indigo-50/30 py-6 px-8 italic rounded-r-2xl mb-10 text-slate-700 text-lg md:text-xl leading-relaxed">
        {children}
      </blockquote>
    )
  };
          ...
          <article className="flex-1 min-w-0 pb-32">
            ...
            <header className="mb-16">
                <div className="flex flex-wrap items-center gap-6 text-xs text-slate-400 mb-8 font-bold uppercase tracking-wider">
                    ...
                    <span className="flex items-center gap-1.5 text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg border border-indigo-100">
                        <Clock size={12} /> {readTime} min de leitura
                    </span>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-12">
                    {post.tags.map(tag => (
                        <span key={tag} className="px-3 py-1 bg-slate-100 text-slate-600 text-[9px] font-black uppercase tracking-widest rounded-full border border-slate-200">
                            #{tag}
                        </span>
                    ))}
                </div>
            ...
            {post.coverImage && (
                <div className="relative mb-20 group">
                    ...
                    <img 
                        src={post.coverImage} 
                        className="relative w-full rounded-[2rem] shadow-2xl aspect-video object-cover" 
                        alt={post.title} 
                    />
                </div>
            )}
            ...
            <div className="max-w-[75ch] mx-auto lg:mx-0">
                {/* Tailwind Typography Optimization */}
                ...
                <div className="mt-24 p-10 bg-slate-900 rounded-[2.5rem] text-center text-white relative overflow-hidden shadow-2xl">
                    <div className="relative z-10">
                        <h4 className="font-black text-2xl mb-3">Gostou deste artigo?</h4>
                        <p className="text-slate-400 mb-8 max-w-sm mx-auto text-sm">Ajude a espalhar o conhecimento sobre inteligência artificial com sua rede.</p>
                        <button className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-indigo-500 transition-all flex items-center gap-3 mx-auto shadow-xl">
          ...
          <aside className="w-full lg:w-80 lg:shrink-0 lg:order-last mb-12 lg:mb-0">
              <div className="sticky top-28 space-y-8">
                  <div className="bg-white rounded-[2rem] p-8 text-slate-900 shadow-xl border border-slate-100 relative overflow-hidden">
                      <div className="relative z-10">
                          <h5 className="font-black text-[10px] uppercase tracking-widest mb-6 text-indigo-500 flex items-center gap-2">
                             <BookOpen size={12} /> Leitura Atual
                          </h5>
                  ...
                  {toc.length > 0 && (
                    <div className="bg-white rounded-[2rem] border border-slate-100 p-8 shadow-sm">
                        <h4 className="font-black text-slate-900 mb-6 flex items-center gap-3 uppercase tracking-widest text-[10px] text-indigo-500">
                            <List size={14} /> Tópicos
                        </h4>
                        <nav className="space-y-1">
                            {toc.map((item) => (
                                <a
                                    ...
                                    className={`group flex items-start gap-3 py-2.5 text-[11px] transition-all duration-300 rounded-xl px-3 -mx-3 ${
                                        item.id === activeId 
                                ...
                                >
                                    <ChevronRight 
                                        ...
                                        className={`mt-0.5 shrink-0 transition-all ${
                                            item.id === activeId ? 'opacity-100 scale-100' : 'opacity-0 scale-50'
                                        }`} 
                                    />
